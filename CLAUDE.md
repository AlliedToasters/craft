...
- `craft/spawn.py` — `random_spawn(range_blocks, ...)` biome-aware spectator column-spawn (12x retry). Per attempt: **spectator** at high Y (`probe_y=320`) forces chunk-gen with noclip (no encasement during gen) → poll `/scan_blocks` 1×1 column until terrain appears → locate true surface (`_surface_from_column`, skips air/leaves/logs) → **switch to survival** + double-TP onto surface (collision places the player; a spectator TP would sink them since homunculus spectator drifts downward) → settle + HP probe. Rejects water/lava/bad_biome/gen_timeout/damage_in_survival. This *measures* the surface instead of guessing a drop_y, so high-terrain biomes (surface y>100) are no longer systematically excluded, and the post-gen race that caused mid-rollout suffocation at y=100 is closed. `BAD_BIOMES` grows only from observed failures (oceans, desert, badlands variants, ice_spikes, frozen_*, windswept_*).
