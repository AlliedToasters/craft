#!/bin/bash
# N=20 concurrent rollouts: pure-qwen daily driver, difficulty=easy,
# milestones M1+M2 enabled (default chain), 30 turns minimal.
#
# Per CLAUDE.md daily driver baseline + fleet scaling memory (C~20 sweet
# spot). 20 agents fan out from a single bash; each rollout retargets via
# HOMUNCULUS_PORT + MC_PLAYER_NAME so craft.config wires the whole stack.
#
# Prereq: agents 0..19 already launched (launch_agent.sh N) and in-world.
# MC server console relay running at $MC_SERVER_CMD_BASE (port 4747).
#
# Output:
#   results/bigN20-easy-qwen-<ts>/
#     _orchestrator.log          per-agent start/end ticks + tally
#     agent<N>-<ts>.jsonl        rollout transcript
#     agent<N>-<ts>.log          agent.py stdout/stderr
#
# Usage:
#   ./scripts/bigN20_easy_qwen.sh
...
# Default milestone chain (M1_iron_goal, M2_diamond_goal) — leave
# CRAFT_MILESTONES unset so resolve_milestones() returns the full list.
unset CRAFT_MILESTONES
# Same for nudges: take the default chain (food_low + night_bed tiers).
unset CRAFT_NUDGES

TODAY=$(date '+%Y%m%d-%H%M%S')
OUT="results/bigN20-easy-qwen-${TODAY}"
mkdir -p "$OUT"

INDEX="$OUT/_orchestrator.log"
{
    echo "[$(date '+%H:%M:%S')] BIGN20 EASY QWEN starting (out=$OUT)"
